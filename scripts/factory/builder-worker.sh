...
quality_gate() {
    ...
    while [[ $attempt -lt $max_fix_attempts ]]; do
        ...
        local fix_file
        fix_file="$(mktemp /tmp/claude-fix-XXXXXX)"
...
build_story() {
    ...
    local _backlog_tmp
    _backlog_tmp="$(mktemp)"
    cp "$BACKLOG_FILE" "$_backlog_tmp"
    git -C "$REPO_ROOT" stash 2>/dev/null || true
    git -C "$REPO_ROOT" checkout "$BASE_BRANCH"
    retry $MAX_RETRIES git -C "$REPO_ROOT" fetch origin "$BASE_BRANCH"
    git -C "$REPO_ROOT" reset --hard "origin/$BASE_BRANCH"
    git -C "$REPO_ROOT" stash pop 2>/dev/null || true
    cp "$_backlog_tmp" "$BACKLOG_FILE"
    rm -f "$_backlog_tmp"
...
main() {
    ...
    while true; do
        ...
        # Wait for reviewer + merger to fully complete this story before starting the next one
        log_info "Story $story_id built — waiting for review + merge to complete..."
        while true; do
            local current_status
            current_status="$(jq -r ".[] | select(.id == \"$story_id\") | .status" "$BACKLOG_FILE" 2>/dev/null || echo "")"
            if [[ "$current_status" == "done" ]]; then
                log_info "Story $story_id done — picking up next story"
                break
            elif [[ "$current_status" == "escalated" ]]; then
                log_error "Story $story_id escalated — human intervention required. Stopping builder."
                exit 1
            fi
            log_debug "Story $story_id status=$current_status — waiting ${POLL_INTERVAL_BUILDER}s..."
            sleep "$POLL_INTERVAL_BUILDER"
        done
    done
}
